...
class Tracer {
 ...
 public:
  Tracer(const char *url, const char *endpoint_addr, const int port)
      : tracer_(nullptr), collector_(nullptr), global_span_(nullptr) {
    std::unique_ptr<zipkin::HttpConf> conf(new zipkin::HttpConf(url));
    collector_ = std::unique_ptr<zipkin::HttpCollector>(conf->create());
    tracer_ = std::unique_ptr<zipkin::Tracer>(
        zipkin::Tracer::create(collector_.get()));
  ...
  zipkin::Span *global() { return global_span_; }

  zipkin::Span *span(const char *name) { return global_span_->span(name); }

  zipkin::Endpoint endpoint() {
    return zipkin::Endpoint("service_name", &endpoint_addr_);
  }

  std::unique_ptr<zipkin::HttpCollector> &collector() { return collector_; }
